...
def add_promotion(text: str):
    ...
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO promotions (text) VALUES (?)
    ''', (text,))
    conn.commit()
    conn.close()
...
def get_current_promotion() -> Optional[str]:
    ...
    cur = conn.cursor()
    cur.execute('''
        SELECT text FROM promotions ORDER BY created_at DESC LIMIT 1
    ''')
    row = cur.fetchone()
    conn.close()
    return row[0] if row else None
...
def get_admin_keyboard():
    keyboard = [
        ...
        [KeyboardButton("➕ Добавить акцию"), KeyboardButton("🔄 Закончить диалог")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def handle_text_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📅 Запись":
        ...
        if promo:
            await update.message.reply_text(f"🎉 *Текущая акция:*\n\n{promo}", parse_mode='Markdown')
        else:
    ...
    # Обработка кнопок мастера/админа
        ...
        return
    elif text == "➕ Добавить акцию" and is_admin(user.id):
        context.user_data['awaiting_promo'] = True
        await update.message.reply_text(
            "📝 Введите текст новой акции (можно с эмодзи и форматированием):"
        )
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если админ вводит текст акции
    if context.user_data.get('awaiting_promo') and is_admin(user.id):
        add_promotion(text)
        context.user_data.pop('awaiting_promo')
        await update.message.reply_text("✅ Акция успешно добавлена! Клиенты смогут её увидеть.")
        log_message(user.id, f"[ДОБАВЛЕНА АКЦИЯ] {text}", is_master=False)
        # Показываем меню админа снова
        await show_main_menu(update, context)
        return
